...
class OGReplayBuffer(ReplayBuffer):
  ...
  def __iter__(self):
    while True:
      ...
      distances = np.random.uniform(0, 1, size=(batch_size, batch_length)) * 0.15 
      # 10% next state goals
      final_pos = np.expand_dims(self._episode_lens[b_indices], axis=1) - 1

      goal_pos = ((final_pos - t_ranges) * distances).astype(int) + t_ranges + 1
      # offset = np.random.geometric(p=1 - 0.9, size=batch_size) * np.where(np.random.rand(batch_size)<0.2, 0, 1)
      ...
      for k in self._complete_eps:
        ...
        if k =='reward':
          ...
          steps_to_go = (goal_pos - (t_ranges -1)) / final_pos 
          chunk[k] = steps_to_go
        else:
          chunk[k] = np.stack([self._complete_eps[k][b][t] for b,t in zip(b_indices, t_ranges)])
      # Goal observation
