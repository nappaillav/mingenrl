...
class OfflineWMBuffer(OfflineBuffer):
    ...
    def sample(self, batch_size, gc_negative=True):
    ...
    def __init__(self, state_dim, action_dim, 
        ...
        if obs_type == 'pixel':
            ...
            self.encoder = Encoder(inp_channel=C, 
                                    ...
                                    layer_norm=True, 
                                    out_emb = hdim)
            feature_layer = (1536+hdim) if goal else hdim
        else:
...
class Agent(object):
    def __init__(self, state_dim, action_dim, max_action, wm,
        ...
        self.critic = WMCritic(state_dim, action_dim, hp.zs_dim, hp.critic_hdim, hp.critic_activ, True, obs_type).to(self.device)
    ...
    def train(self):
        ...
        if self.gc:
            state, action, next_state, reward, not_done, goal, weight = self.replay_buffer.sample(self.hp.batch_size)
        else:
        ...
        with torch.no_grad():
            ...
            zsa = self.wm.Zsa(state, action).detach()
            # if self.offline:
            #     noise = 0.0
            # else:
            ...
            next_zsa = self.wm.Zsa(state, action, next_state, next_action).detach() # here we already have the deter
            Q_target = self.critic_target(next_zsa, goal).min(1,keepdim=True)[0]
        ...
        if self.training_steps % self.hp.policy_freq == 0:
            ...
            zsa = self.wm.Zsa(state, actor).detach() 
            Q = self.critic(zsa, goal)
            actor_loss = -Q.mean()
            if self.offline:
                # actor_loss = weight * (actor_loss + self.hp.lmbda * Q.abs().mean().detach() * F.mse_loss(actor, action))
                # weighted_qval = (weight * -Q).mean()
                # offline_coef = Q.abs().mean().detach()
                bc_loss = self.hp.lmbda * Q.abs().mean().detach() * F.mse_loss(actor , action) # hyper parameter
                # weighted_qval / offline_coef
