  ...
  def act(self, obs, meta, step, eval_mode, state, batch=False, compute_loss=False):
    ...
    if batch:
      ...
      for timestep in range(T-1):
        ...
        if self.gc:
          # feat_dim = task_cond.shape[-1]
          task_cond_feat = task_cond[timestep, :]
        feat = torch.cat([feat, task_cond_feat], -1) if self.gc else feat
        if eval_mode:
      ...
      if eval_mode:
        action = self._acting_behavior.actor(feat)
        # actor = self._acting_behavior.actor(feat) # Goal encoder 
        # try:
        #   action = actor.mean 
        # except:
    ...
    return action/np.linalg.norm(action), new_state
  ...
  def update_acting_behavior(self, state=None, outputs=None, metrics={}, data=None, reward_fn=None):
    ...
    if outputs is not None:
      ...
      if self.gc:
        ...
        # start = {'goal':stop_gradient(goal_emb)}
        feat_dim = goal_emb.shape[-1]
        task_cond = stop_gradient(goal_emb).reshape(-1, feat_dim)
...
class WorldModel(Module):
  ...
  def video_pred(self, data, key, nvid=8):
    ...
    states, _ = self.rssm.observe(
        embed[:nvid, :n], data['action'][:nvid, :n], data['is_first'][:nvid, :n])
    recon = decoder(self.decoder_input_fn(states))[key].mean[:nvid] # mode
    init = {k: v[:, -1] for k, v in states.items()}
    prior = self.rssm.imagine(data['action'][:nvid, n:], init)
    prior_recon = decoder(self.decoder_input_fn(prior))[key].mean # mode
    model = torch.clip(torch.cat([recon[:, :n] + 0.5, prior_recon + 0.5], 1), 0, 1)
    error = (model - truth + 1) / 2
    video = torch.cat([truth, model, error], 3)
    B, T, C, H, W = video.shape
    return video 

  def goal_emb(self, goal):
    return self.encoder({'goal': goal})
  
  def Zsa(self, obs, action, next_obs=None, next_action=None):
    obs = obs.unsqueeze(1) if len(obs.shape) == 4 else obs
    action = action.unsqueeze(1) if len(action.shape) == 2 else action
    if not next_obs is None:
      obs = torch.concat((obs, next_obs.unsqueeze(1)), 1)
      action = torch.concat((action, next_action.unsqueeze(1)), 1)
    B, T, _ = action.shape
    is_first = torch.full((B, T), False, dtype=torch.bool, device='cuda')
    embed = self.encoder({'observation':obs})
